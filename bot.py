    ...
    try:
        ...
        if len(msg_words) == 2:
            ...
            if int(outcome) in [3, 4, 5]:
                change_money(username, (int(0.66 * stake_money)))
                await message.reply(
                    f"🥳 Гол!\n\nВ твоём кошельке внезапно оказались дополнительные {int(0.66 * stake_money)} 💵.\nЕсли уверен, что удача тебя не подведёт, можешь пнуть мяч ещё разок.",
                    reply_markup=markups.to_menus)
            else:
                change_money(username, -stake_money)
                await message.reply(
...
async def basketball(message: types.Message):
    ...
    try:
        ...
        if len(msg_words) == 2:
            ...
            if int(outcome) in [4, 5]:
                change_money(username, (int(1.5 * stake_money)))
                await message.reply(
                    f"🥳 Попал!\n\nВ твоём кошельке внезапно оказались дополнительные {int(1.5 * stake_money)} 💵.\nЕсли уверен, что удача тебя не подведёт, можешь кинуть мяч ещё разок.",
                    reply_markup=markups.to_menus)
            else:
                change_money(username, -stake_money)
                await message.reply(
                    f"😰 Мимо!\n\nТвои {stake_money} 💵 внезапно пропали из кошелька.\nМожет, получится отыграться, если кинуть мяч ещё разок, хотя, с другой стороны, можно проиграть ещё больше. Выбор за тобой.",
                    reply_markup=markups.to_menus)
        else:
            await message.reply(
                f'С твоей командой что-то не так. Вот как её надо вводить:\n/basketball <твоя ставка>\n\nНапример:\n/basketball {randint(100, 10000)}',
                reply_markup=markups.to_menus)
    except:
        await message.reply(
            f'С твоей командой что-то не так. Вот как её надо вводить:\n/basketball <твоя ставка>\n\nНапример:\n/basketball {randint(100, 10000)}',
...
async def slotmachine(message: types.Message):
    ...
    try:
        ...
        if len(msg_words) == 2:
            ...
            if int(outcome) in [1, 22, 43, 64]:
                change_money(username, (int(15 * stake_money)))
                await message.reply(
                    f"🥳 Три в ряд!\n\nВ твоём кошельке внезапно оказались дополнительные {int(15 * stake_money)} 💵.\nЕсли уверен, что удача тебя не подведёт, можешь дёрнуть за ручку автомата ещё разок.",
                    reply_markup=markups.to_menus)
            else:
                change_money(username, -stake_money)
                await message.reply(
                    f"😰 Разные слоты!\n\nТвои {stake_money} 💵 внезапно пропали из кошелька.\nМожет, получится отыграться, если дёрнуть за ручку автомата ещё разок, хотя, с другой стороны, можно проиграть ещё больше. Выбор за тобой.",
                    reply_markup=markups.to_menus)
        else:
            await message.reply(
                f'С твоей командой что-то не так. Вот как её надо вводить:\n/slotmachine <твоя ставка>\n\nНапример:\n/bowling {randint(100, 10000)}',
                reply_markup=markups.to_menus)
    except:
        await message.reply(
            f'С твоей командой что-то не так. Вот как её надо вводить:\n/slotmachine <твоя ставка>\n\nНапример:\n/bowling {randint(100, 10000)}',
...
async def darts_rules(message: types.Message):
    await bot.send_message(message.from_user.id,
...
async def soccer_rules(message: types.Message):
    await bot.send_message(message.from_user.id,
...
@dp.callback_query_handler(text='basketball_rules')
...
@dp.callback_query_handler(text='slotmachine_rules')
